...
type PixelBlastProps = {
  ...
  pixelSize?: number;
  /** CSS variable names to use as pixel colors, e.g. ["--electron", "--primary"]. A random one is picked per pixel. */
  colors?: string[];
  className?: string;
  style?: CSSProperties;
  antialias?: boolean;
  ...
  if (opts.noiseAmount > 0) {
    ...
    const noiseEffect = new Effect(
      ...
      "uniform float uTime; uniform float uAmount; float hash(vec2 p){ return fract(sin(dot(p, vec2(127.1,311.7))) * 43758.5453);} void mainUv(inout vec2 uv){} void mainImage(const in vec4 inputColor,const in vec2 uv,out vec4 outputColor){ float n=hash(floor(uv*vec2(1920.0,1080.0))+floor(uTime*60.0)); float g=(n-0.5)*uAmount; outputColor=inputColor+vec4(vec3(g),0.0);} ",
      {
        uniforms: new Map<string, Uniform>([
          ["uTime", new Uniform(0)],
          ["uAmount", new Uniform(opts.noiseAmount)],
        ]),
      }
    );
...
void main(){
  ...
  // Pick a color per pixel using a stable hash independent from jitter
  float hc = fract(sin(dot(floor(fragCoord / uPixelSize), vec2(92.3, 437.2))) * 31416.5359);
  int colorIdx = clamp(int(hc * float(uColorCount)), 0, MAX_COLORS - 1);
...
/**
 * Resolves CSS variable names (e.g. "--electron") to Color instances
 * by reading computed styles from the document root.
 */
const resolveCssColors = (varNames: string[]): Color[] => {
  ...
  return varNames.map((name) => {
    ...
    // Use a temp element so the browser normalises oklch/hsl/etc. to rgb()
...
/** Builds a padded MAX_COLORS array of Color, cycling through resolved colors. */
const buildColorArray = (resolved: Color[]): Color[] =>
  Array.from(
    { length: MAX_COLORS },
    (_, i) => resolved[i % resolved.length] ?? new Color(1, 1, 1)
  );

// ---------------------------------------------------------------------------
// ThreeInstance lifecycle helpers
// ---------------------------------------------------------------------------

const teardownThreeInstance = (
  t: ThreeInstance,
...
type EffectWithUniforms = Effect & { uniforms: Map<string, Uniform> };

/** Propagates the current uTime value to all composer pass effects. */
const syncTimeToComposerPasses = (
  passes: EffectComposer["passes"],
  timeValue: number
): void => {
  for (const p of passes) {
    const pass = p as { effects?: EffectWithUniforms[] };
    if (pass.effects) {
...
/** Hot-updates all uniforms on an already-initialised ThreeInstance. */
const applyHotUniforms = (
  t: ThreeInstance,
  props: {
    variant: PixelBlastVariant;
    pixelSize: number;
  ...
  const resolvedColors = resolveCssColors(props.colors);
  t.uniforms.uColors.value = buildColorArray(resolvedColors);
  t.uniforms.uColorCount.value = resolvedColors.length;
  ...
  if (t.liquidEffect) {
    const liqEffect = t.liquidEffect as EffectWithUniforms;
...
const PixelBlast: FC<PixelBlastProps> = ({
  ...
  useEffect(() => {
    ...
    if (mustReinit) {
      if (threeRef.current) {
        teardownThreeInstance(threeRef.current, container);
        threeRef.current = null;
      }
      threeRef.current = buildThreeInstance(container, {
        antialias,
        transparent,
        pixelSize,
        patternScale,
      ...
      applyHotUniforms(threeRef.current, {
        variant,
        pixelSize,
        colors,
        patternScale,
    ...
    return () => {
      // Cleanup only on unmount, not on hot-reload re-runs
...
// ---------------------------------------------------------------------------
// ThreeInstance factory (defined after component to keep it out of module scope
// globals but close to where it is used)
// ---------------------------------------------------------------------------

type BuildThreeInstanceOpts = {
  antialias: boolean;
  transparent: boolean;
  pixelSize: number;
  patternScale: number;
  ...
  threeRef: { current: ThreeInstance | null };
};

function buildThreeInstance(
  container: HTMLElement,
  opts: BuildThreeInstanceOpts
): ThreeInstance {
  const canvas = document.createElement("canvas");
  ...
  const resolvedColors = resolveCssColors(opts.colors);
  const colorArray = buildColorArray(resolvedColors);

  const uniforms: ThreeUniforms = {
    uResolution: { value: new Vector2(0, 0) },
    uTime: { value: 0 },
    uColors: { value: colorArray },
    uColorCount: { value: resolvedColors.length },
    uClickPos: {
  ...
  const setSize = () => {
    ...
    if (opts.threeRef.current?.composer) {
      opts.threeRef.current.composer.setSize(
        renderer.domElement.width,
        renderer.domElement.height
      );
    }
  ...
  const { composer, touch, liquidEffect } = setupPostProcessing(
    renderer,
    scene,
    camera,
    {
      liquid: opts.liquid,
      liquidRadius: opts.liquidRadius,
      liquidStrength: opts.liquidStrength,
      liquidWobbleSpeed: opts.liquidWobbleSpeed,
      noiseAmount: opts.noiseAmount,
    }
  );
  ...
  const onPointerDown = (e: PointerEvent) => {
    ...
    const ix = opts.threeRef.current?.clickIx ?? 0;
    uniforms.uClickPos.value[ix].set(fx, fy);
    uniforms.uClickTimes.value[ix] = uniforms.uTime.value;
    if (opts.threeRef.current) {
      opts.threeRef.current.clickIx = (ix + 1) % MAX_CLICKS;
    }
  };
  ...
  let raf = 0;
  const animate = () => {
    if (opts.autoPauseOffscreen && !opts.visibilityRef.current.visible) {
      raf = requestAnimationFrame(animate);
      return;
    }
    uniforms.uTime.value =
      timeOffset + clock.getElapsedTime() * opts.speedRef.current;

    if (liquidEffect) {
      const u = (liquidEffect as EffectWithUniforms).uniforms.get("uTime");
      if (u) {
    ...
    if (composer) {
      touch?.update();
      syncTimeToComposerPasses(composer.passes, uniforms.uTime.value);
      composer.render();
    } else {
